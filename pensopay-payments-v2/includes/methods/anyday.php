...
class Pensopay_Payments_V2_Methods_Anyday extends Pensopay_Payments_V2_Methods_Abstract {
	public function __construct() {
		$this->id = 'pensopay_gateway_anyday_split';
		...
		$this->method_title = __( 'Anyday', Pensopay_Payments_V2_Gateway::TEXT_DOMAIN );
		...
		$this->method_description = __( 'Accept Anyday Split payments through pensopay', Pensopay_Payments_V2_Gateway::TEXT_DOMAIN );
	...
	public function init_form_fields() {
		$this->form_fields = apply_filters(
			'pensopay_payments_anyday_split_settings',
			array(
				'enabled'     => array(
					'title'   => __( 'Enable/Disable', Pensopay_Payments_V2_Gateway::TEXT_DOMAIN ),
					'label'   => __( 'Enable ANYDAY Split', Pensopay_Payments_V2_Gateway::TEXT_DOMAIN ),
					'type'    => 'checkbox',
					'default' => 'no',
				),
				'title'       => array(
					...
					'default'     => __( 'ANYDAY Split', Pensopay_Payments_V2_Gateway::TEXT_DOMAIN ),
					'desc_tip'    => true,
				),
				'description' => array(
					...
					'default'     => __( 'Pay with ANYDAY Split', Pensopay_Payments_V2_Gateway::TEXT_DOMAIN ),
					'desc_tip'    => true,
				)
			)
		);
	...
	public function apply_gateway_icons( $icon, $id ) {
		if ( $id == $this->id ) {
			$icon = $this->create_gateway_icon( 'anyday-split' );
		}
	...
	public function should_disable_gateway( $gateways ) {
		if ( isset( $gateways[ $this->id ] ) && is_checkout() && ( $cart = WC()->cart ) ) {
			...
			$allowed_currencies = array( 'DKK' );

			if ( ( $cart_total < $gate_min ) || $cart_total < $cart_min || ! in_array( $context_currency, $allowed_currencies, true ) ) {
				unset( $gateways[ $this->id ] );
			}
		}
