...
class Pensopay_Payments_V2_Methods_StripeIdeal extends Pensopay_Payments_V2_Methods_Abstract {
	public function __construct() {
		$this->id = 'pensopay_gateway_ideal';
		...
		add_filter( 'pensopay_payments_request_body', array( $this, 'set_ideal_fields' ) );
	}

	public function set_ideal_fields( $request ) {
//		if ($request['facilitator'] && $request['facilitator'] == 'ideal') {
...
//		}
	...
	public function init_form_fields() {
		$this->form_fields = apply_filters(
			'pensopay_payments_ideal_settings',
			array(
				'enabled'     => array(
	...
	public function apply_gateway_icons( $icon, $id ) {
		if ( $id == $this->id ) {
			$icon = $this->create_gateway_icon( 'ideal' );
		}
