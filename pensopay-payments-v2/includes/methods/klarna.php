...
class Pensopay_Payments_V2_Methods_Klarna extends Pensopay_Payments_V2_Methods_Abstract {
	public function __construct() {
		$this->id = 'pensopay_gateway_payments';
		...
		$this->method_title = __( 'Klarna Payments', Pensopay_Payments_V2_Gateway::TEXT_DOMAIN );
	...
	public function init_form_fields() {
		$this->form_fields = apply_filters(
			'pensopay_payments_klarna_payments_settings',
			array(
				'enabled'     => array(
					'title'   => __( 'Enable/Disable', Pensopay_Payments_V2_Gateway::TEXT_DOMAIN ),
					'label'   => __( 'Enable Klarna Payments', Pensopay_Payments_V2_Gateway::TEXT_DOMAIN ),
					'type'    => 'checkbox',
					'default' => 'no',
				),
				'title'       => array(
					...
					'default'     => __( 'Klarna Payments', Pensopay_Payments_V2_Gateway::TEXT_DOMAIN ),
					'desc_tip'    => true,
				),
				'description' => array(
					...
					'default'     => __( 'Pay with Klarna Payments', Pensopay_Payments_V2_Gateway::TEXT_DOMAIN ),
					'desc_tip'    => true,
				)
			)
		);
	...
	public function should_disable_gateway( $gateways ) {
		if ( isset( $gateways[ $this->id ] ) && is_checkout() && ( $cart = WC()->cart ) ) {
			...
			return $gateways; //Currently completely disabled
